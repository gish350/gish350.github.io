...
```csharp
	    managementObjectSearcher = new ManagementObjectSearcher("Select * from Win32_ComputerSystem");
	  	using (ManagementObjectCollection managementObjectCollection = managementObjectSearcher.Get())
			...
			return false;
```
Далее, перейдя в класс **qXK**, код стал слишком запутанным. До этого места обфускация была несильной, но перейдя в этот класс я обнаружил следующий листинг: 

![Animation.gif](/Animation.gif)

Я нашел статью где описано написание деобфускатора для de4dot на похожем образце AgentTesla, где происходит [PLACEHOLDER]. Вот что получилось в результате
